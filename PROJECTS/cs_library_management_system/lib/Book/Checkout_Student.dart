...
// Check Book Section
class Checkout_Student extends StatefulWidget {
  ...
  @override
  State<Checkout_Student> createState() => _Checkout_StudentState();
}

class _Checkout_StudentState extends State<Checkout_Student> {
  ...
  Future checkOutBookStudentData(String booktitle1, String edition1, String publishedyear1,
      String isbnno1, String registerno1) async
  {
    await BookDBHelper.bookcountreduce(isbnno1);
    await BookDBHelper.studentcountreduce(registerno1);
    var checkdate = DateTime.now();
    var renewdate = checkdate.add(const Duration(days: 30));
    print(renewdate);

    final checkout = BOOKSTUDENT(
        bookname: booktitle1,
        isbnno: isbnno1,
        id: m.ObjectId(),
        edition: edition1,
    ...
    final result = await BookDBHelper.CheckoutStudentBook(checkout);
    if (result == "True") {
  ...
  Future bookCountUpdate(String isbnno3) async {
    ...
    if(data == "True")
    {
      studentCountUpdate(registerno.text.toUpperCase());
  ...
  Future studentCountUpdate(String register) async {

    final data = await BookDBHelper.studentCountUpdate(register);
    if(data == "True")
    {
      checkOutBookStudentData(bookTitle.text.toUpperCase(),
    ...
    {
      customErrorToast("Student Not Found...", context);
      print("student");
    }
  }
